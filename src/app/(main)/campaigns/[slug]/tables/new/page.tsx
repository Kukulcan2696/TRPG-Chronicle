...
export default async function NewTablePage({ params }: PageProps) {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold">新建随机表</h1>
        <p className="text-muted-foreground">为「{campaign.title}」创建掷表</p>
      </div>

      <Card>
        <CardContent className="pt-6">
          {/* createRandomTable.bind(null, slug) 将 campaignSlug 预绑定 */}
          <form action={createRandomTable.bind(null, slug)} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="title">表名</Label>
              <Input id="title" name="title" placeholder="例：随机遭遇表" required />
            </div>

            <div className="space-y-2">
              <Label htmlFor="description">描述</Label>
              <Input id="description" name="description" placeholder="d6 遭遇表" />
            </div>
