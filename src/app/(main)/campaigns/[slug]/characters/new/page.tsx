...
export default async function NewCharacterPage({ params }: PageProps) {
  ...
  // We render template fields client-side based on selected system.
  // For SSR, show all templates hidden and reveal via JS.
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div><h1 className="text-2xl font-bold">创建角色</h1><p className="text-muted-foreground">为「{campaign.title}」创建新角色</p></div>
      <Card><CardContent className="pt-6">
        <form action={createCharacter.bind(null, slug)} className="space-y-6">
          <div className="space-y-2"><Label htmlFor="name">角色名</Label><Input id="name" name="name" placeholder="例：甘道夫·灰袍" required /></div>
          ...
          <div className="space-y-2"><Label htmlFor="bio">背景简介</Label><Textarea id="bio" name="bio" placeholder="角色的背景故事概要..." rows={3} /></div>
          <div className="flex items-center gap-2"><Switch id="isPublic" name="isPublic" value="true" defaultChecked /><Label htmlFor="isPublic">公开角色卡</Label></div>

          {/* Template fields - hidden by default, shown per system via JS */}
      ...
      {/* Simple JS to show/hide template fields */}
