 ...
 * 加入战役页面
 * 
 * 通过邀请码加入战役，无需登录时显示登录提示。
 * URL 格式：/join?code=XXXXXXXX
 * 
 * 流程：
 * 1. 读取 query 参数中的邀请码
 * 2. 校验用户已登录
 * 3. 查找战役并添加为 PLAYER 成员
 * 4. 跳转到战役概览页
 */
...
export default async function JoinPage({ searchParams }: PageProps) {
  ...
  try {
    ...
    // joinCampaignByCode 内部会 redirect，通常不会执行到这里
    return null;
  } catch (error: any) {
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <Card className="max-w-md w-full text-center">
          <CardHeader>
            <CardTitle>加入失败</CardTitle>
            <CardDescription>{error.message || "邀请码无效或已过期"}</CardDescription>
          </CardHeader>
          <CardContent>
            <Link href="/dashboard" className={buttonVariants({ variant: "outline" })}>
              返回仪表盘
