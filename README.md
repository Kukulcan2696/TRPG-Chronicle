...
### 管理后台（仅 ADMIN 角色）
- 系统统计概览（用户/战役/战报/角色/掷骰数）
- 用户管理：查看、修改角色（PLAYER / DM / ADMIN）
- 战役管理：查看所有战役，删除任意战役
- 入口：导航栏头像下拉菜单 → "管理后台"（仅管理员可见）
- 首次设置管理员：`npx prisma studio` → User 表 → 改 role 为 ADMIN
### 个人设置
- 编辑用户名
- 上传/更换头像
- 修改密码（需验证当前密码）
...
### 管理后台
- 仅 ADMIN 角色可访问（需手动在数据库设置）
- 系统统计概览（用户、战役、战报等）
- 用户管理：查看列表、修改角色（PLAYER / DM / ADMIN）
- 战役管理：查看列表、删除战役
